...
export default function DetailsClass() {
  ...
  const [shoow, setShoow] = useState(true);
...
await init3();
setCount(count+1);
alert("event deleted please reload to not see the event");
  }
  ...
  const init1 = async (id)=>{
    ...
    CurrSche.push({uniqid,classid:FindId()});
...
await init3();
alert("Schedule  Created please reload to  see the event");
  }
                  ...
                  <>
                    {show === true ? (
                      <>
                        {te.Student.map((e) => {
                          return (
                            <Typography component="" variant="subtitle1" align="center" key={JSON.stringify(e)}>
                            {e}
                            </Typography>
